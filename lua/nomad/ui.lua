...
function M.update_sidebar(data)
  ...
  local jobs_section = NuiTree.Node({ text = "📦 Jobs (" .. #jobs .. ")", jobs_section = true }, jobs_children)
  ...
  local nodes_section =
    NuiTree.Node({ text = "🖥️  Nodes (" .. #nodes .. ")", nodes_section = true }, nodes_children)
      ...
      for _, node in ipairs(dc_info.nodes) do
        ...
        if node.allocations and #node.allocations > 0 then
          ...
          for i, alloc in ipairs(node.allocations) do
            ...
            local alloc_line = string.format(
              "      %s %-15s %-20s %4dMHz %4dMB [logs]",
              job_icon,
              job_name,
              alloc_name,
              cpu_val,
...
-- Helper function to add basic job info to content
local function add_basic_job_info(content, job)
  table.insert(content, "Job: " .. job.ID)
  table.insert(content, "Status: " .. (job.Status or "unknown"))
  table.insert(content, "Type: " .. (job.Type or "unknown"))
  table.insert(content, "Namespace: " .. (job.Namespace or "default"))
...
-- Helper function to add task groups info to content
local function add_task_groups_info(content, details)
  if details.TaskGroups then
    table.insert(content, "Task Groups: " .. #details.TaskGroups)
    for _, tg in ipairs(details.TaskGroups) do
      table.insert(content, "  - " .. tg.Name .. " (Count: " .. (tg.Count or 1) .. ")")
...
-- Helper function to add allocations info to content
local function add_allocations_info(content, details)
  if details.Allocations then
    table.insert(content, "")
    table.insert(content, "=== Allocations (" .. #details.Allocations .. ") ===")
    for i, alloc in ipairs(details.Allocations) do
      table.insert(
        content,
        "  " .. i .. ". " .. (alloc.ID or "unknown") .. " [" .. (alloc.ClientStatus or "unknown") .. "]"
      )
...
-- Helper function to build job details content
local function build_job_details_content(job, details)
  local content = {}
  add_basic_job_info(content, job)

  if details then
    table.insert(content, "")
    table.insert(content, "=== Details ===")
    add_task_groups_info(content, details)
    add_allocations_info(content, details)
  end

  return content
end

-- Helper function to setup job details popup keymaps
local function setup_job_details_keymaps(popup, job, details)
  -- Close on q or Escape
  vim.keymap.set("n", "q", function()
    popup:unmount()
    M.details_popup = nil
...
function M.show_job_details_enhanced(job, details)
  ...
  local content = build_job_details_content(job, details)
  ...
  setup_job_details_keymaps(M.details_popup, job, details)
end
